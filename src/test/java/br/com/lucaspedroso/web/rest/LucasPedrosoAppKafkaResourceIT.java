...
import br.com.lucaspedroso.config.KafkaProperties;
...
class LucasPedrosoAppKafkaResourceIT {
    ...
    void setup() {
        ...
        LucasPedrosoAppKafkaResource kafkaResource = new LucasPedrosoAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/lucas-pedroso-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/lucas-pedroso-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
